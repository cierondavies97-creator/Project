...
def score_trades(
    ...
    if decisions_ctx is not None and not decisions_ctx.is_empty():
        ...
        if join_keys and ctx_cols:
            base = base.join(decisions_ctx.select(join_keys + ctx_cols), on=join_keys, how="left")

    if "anchor_ts" not in base.columns and "entry_ts" in base.columns:
