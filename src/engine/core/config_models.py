...
def _cluster_instruments(retail: RetailConfig, cluster_id: str) -> list[str]:
    instruments = [inst.instrument for inst in retail.instruments if inst.cluster_id == cluster_id]
    if not instruments:
...
def build_cluster_plan(
    ...
    if not instruments:
        raise ValueError(f"After applying snapshot filters, no instruments remain for cluster_id='{cluster_id}'")
    ...
    if not anchor_tfs:
        raise ValueError("ClusterPlan produced no anchor_tfs (check retail.yaml and snapshot.data_slice.anchor_tfs)")

    if not entry_tfs:
        raise ValueError("ClusterPlan produced no entry_tfs (check retail.yaml and snapshot.data_slice.tf_entries)")
...
def load_features_registry(
    ...
    for family_id, cfg in raw.items():
        ...
        tables_raw = cfg.get("tables", None)
        if tables_raw is None:
            if "table" not in cfg:
                raise ValueError(f"Registry entry for family_id='{family_id}' must include 'table' or 'tables'")
            tables = [str(cfg["table"])]
        else:
            if not isinstance(tables_raw, list):
                raise ValueError(f"'tables' for family_id='{family_id}' must be a list")
            tables = [str(x) for x in tables_raw if str(x).strip()]
            if not tables:
                raise ValueError(f"'tables' for family_id='{family_id}' must not be empty")
...
def families_for_table(
    ...
    *,
    ...
    for entry in registry.families.values():
        if table not in (entry.tables or [entry.table]):
            continue
